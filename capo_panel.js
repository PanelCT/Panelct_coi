...
async function mostraSessioneAttiva() {
  try {
    ...
    testoSessione.textContent = sessione.codice_sessione;

    await aggiornaConteggioSchede(sessione.id);

  } catch (errore) {
    testoSessione.textContent = "Errore nel caricamento";
    mostraMessaggio(
      "Errore lettura sessione: " + errore.message,
      true
    );
  }
}
...
async function creaNuovaSessione(tipoOperazione) {
  const conferma = confirm(
    tipoOperazione === "valutazione"
      ? "Vuoi chiudere la valutazione attuale e iniziarne una nuova?"
      : "Vuoi creare una nuova sessione? Le precedenti verranno disattivate."
  );
  ...
  mostraMessaggio("Creazione della nuova sessione...");

  try {
    const { error: erroreChiusura } = await client
      .from("sessioni")
      .update({ attiva: false })
      .eq("attiva", true);
    ...
    if (erroreChiusura) {
      throw new Error(
        "Impossibile disattivare le sessioni precedenti: " +
    ...
    const { data: nuovaSessione, error: erroreCreazione } =
      await client
        .from("sessioni")
        .insert([
          {
    ...
    if (erroreCreazione) {
      throw new Error(erroreCreazione.message);
    }
    ...
    pulsanteNuovaSessione.disabled = false;
    pulsanteNuovaValutazione.disabled = false;
  }
}
...
function valoreCsv(valore) {
  ...
  const testo = String(valore).replaceAll('"', '""');
...
async function esportaSessioneCorrente() {
  ...
  pulsanteEsporta.textContent = "ESPORTAZIONE IN CORSO...";
  ...
  try {
    ...
    if (!data || data.length === 0) {
      alert(
        "Non sono ancora presenti schede nella sessione attiva."
      );
      return;
    }
    ...
    righe.push(
      intestazioniItaliane.map(valoreCsv).join(";")
    );

    data.forEach(scheda => {
      const riga = colonne.map(colonna =>
    ...
    link.download =
      `${sessione.codice_sessione}-schede.csv`;
    ...
    mostraMessaggio(
      `Esportate ${data.length} schede della sessione corrente.`
    );

  } catch (errore) {
    mostraMessaggio(
      "Errore esportazione: " + errore.message,
      true
    );

  } finally {
    pulsanteEsporta.disabled = false;
    pulsanteEsporta.textContent = "📥 Esporta Excel COI";
  }
}
...
pulsanteNuovaSessione.addEventListener(
  "click",
  () => creaNuovaSessione("sessione")
);

pulsanteNuovaValutazione.addEventListener(
  "click",
  () => creaNuovaSessione("valutazione")
);
...
// Carica automaticamente la sessione attiva
mostraSessioneAttiva();


// Aggiorna il conteggio ogni 15 secondi
setInterval(mostraSessioneAttiva, 15000);
